...
test("Search hotels in Dubai", async ({ page }) => {
  // Load the page and wait for all network calls to settle
  await page.goto("https://phptravels.net/", { waitUntil: "networkidle" });

  // Hotels tab locator
  const hotelsTab = page.locator('a[data-bs-target="#hotels"]');

  // Wait for Hotels tab to be visible and clickable (CI needs this)
  await hotelsTab.waitFor({ state: "visible", timeout: 60000 });
